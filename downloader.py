...
def menu():
    ...
    match option:
        case 1:
            add_track('audio', 'image/tracks', 'xmp3')
        case 2:
...
def add_track(audio_dir, image_dir, csv_dir):
    ...
    while True:
        ...
        if new_artist_name:
            option = input('Download again? (y/yes):\n>>> ')
            option = option.lower()
    ...
    cover = webp_extension(album_name)
...
def add_artist(artist_name, image_dir, csv_dir):
    ...
    cover = webp_extension(artist_name)
...
def webp_extension(name):
    return f'{snake_case(name)}.webp'
